...
bool loadMap(CMap@ _map, const string& in filename)
{
	...
	//gold under tent
	u32 goldUnderTent = cfg.read_u32("gold_under_tent", 0);
	u32 goldStartX = cfg.read_u32("gold_start_x", 12);
	u32 goldStartY = cfg.read_u32("gold_start_y", 20);

	//gold in middle
	u32 midGoldStartDepth = cfg.read_u32("mid_gold_start_depth", 5);
	s32 midGoldStartPosFromMid = cfg.read_s32("mid_gold_start_pos_from_mid", 0);
	u32 midGoldDepth = cfg.read_u32("mid_gold_depth", 7);

	//hole in middle
	s32 holeWidth = cfg.read_s32("hole_width", 0);

	//trees
	s32 tree_limit = cfg.read_s32("tree_limit", 1);
		...
		for (int y = 0; y < height; y++)
		{
			...
			if (n > 0.5f)
			{
				...
				else
				{
					...
					// }
					else if (material_frac > 0.4f && n > 0.5f)
					{
						map.SetTile(offset, CMap::tile_thickstone);
					}
					else if (material_frac > 0.1f && n_plus > 0.6f)
					{
    ...
    if (goldUnderTent > 0)
	{
        ...
        for (uint i = 0; i < goldUnderTent; i++)
		{
    ...
    //middle gold
    uint midGoldStartX = (width / 2) + midGoldStartPosFromMid;
    uint midGoldStartY = ((height * (100 - baseline)) / 100) + midGoldStartDepth;
    int midGoldWidth = midGoldStartPosFromMid * 2;
	...
	{
        for (uint i = 0; i < midGoldDepth; i++)
		for (uint j = 0; j < midGoldWidth; j++)
		{
    ...
    //middle hole
    uint holeStartX = (width / 2) - (holeWidth / 2);
    if (holeWidth > 0)
	{
        for (uint i = 0; i < height; i++)
		for (uint j = 0; j < holeWidth; j++)
		{
			map.SetTile(holeStartX + j + (i * width), CMap::tile_empty);
		}

		//stop plants growing in the hole
        for (uint x = 0; x < holeWidth; x++)
		{
			naturemap[holeStartX + x] = -1;
		}
    }
